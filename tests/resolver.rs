...
fn load_sources_resolver_fixture_catalogues() {
    ...
    let mut ctx = registry.context();
    let input = json!({
        "projectPath": lcod_kernel_rs::core::path::path_to_string(&fixture_root),
        "cacheDir": lcod_kernel_rs::core::path::path_to_string(cache_dir.path()),
        "sourcesPath": lcod_kernel_rs::core::path::path_to_string(&sources_path),
        "resolverConfig": {}
    });
